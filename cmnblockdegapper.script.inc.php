...
define('CMN_VERSION','1.3.0');

xecho('cmnblockdegapper v'.CMN_VERSION."\n");
...
if ($argc == 2) {
  ...
  $result = cmn_cmd_get('/blocksgaps',array("interval"=>"P1M",'testnet'=>$testnet),$response);
  ...
  if (count($gaps) == 0) {
    ...
    foreach($gaps as $id => $gap) {
      ...
      $output = array();
      $result = 0;
      $lastline = exec("/usr/bin/timeout 10 ".CMN_DIR."/chcblockretrieve $uname $gap",$output,$result);
      if ($result == 0) {
        echo "OK";
      }
      else {
