...
// Deal with chaincoin.conf configuration (read/write)
class ChaincoinConfig {

  // Normal chaincoin.conf configuration
  private $config;
  ...
  private function loadconfig() {
    ...
    if (file_exists($this->configfilename)) {
      ...
      $conf = explode("\n",trim($rawconf));
      $magiclen = strlen(ChaincoinConfig::MAGIC);
      for ($x = 0; $x < count($conf); $x++) {
        if ((substr($conf[$x],0,1) == '#') && (substr($conf[$x],0,$magiclen) != ChaincoinConfig::MAGIC)) {
          $lineval = array(0 => $conf[$x]);
        }
        else {
          $lineval = explode('=',$conf[$x]);
        }
        if (substr($lineval[0],0,$magiclen) == ChaincoinConfig::MAGIC) {
  ...
  function __construct($uname) {
    ...
    if (file_exists('/home/'.$uname.'/.bitcore/data/chaincoin.conf')) {
      $this->configfilename = '/home/'.$uname.'/.bitcore/data/chaincoin.conf';
    }
    elseif (file_exists('/etc/chaincoincore/chaincoin.conf')) {
      $this->configfilename = '/etc/chaincoincore/chaincoin.conf';
    }
    else {
      $this->configfilename = '/home/'.$uname.'/.chaincoincore/chaincoin.conf';
    }
    $this->loadconfig();
  ...
  function saveconfig() {
    if ($this->configloaded) {
      ...
      foreach ($this->mnctlcfg as $key => $value) {
        if ($value === false) {
          $rawconf .= ChaincoinConfig::MAGIC.$key."\n";
        }
        else {
          $rawconf .= ChaincoinConfig::MAGIC.$key.'='.$value."\n";
        }
      }
