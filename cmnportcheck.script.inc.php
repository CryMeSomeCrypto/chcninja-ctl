...
if ((!defined('CMN_SCRIPT')) || (CMN_SCRIPT !== true)) {
  die('This is part of the cmnctl script, run it from there.');
}

DEFINE('CMN_VERSION','2.3.0');

// Execute port check commands
function cmn_portcheck_mt(&$commands) {
  ...
  xecho("Executing ".count($commands)." portcheck commands (using ".CMN_THREADS_MAX." threads):\n");
  ...
  while ($done != count($commands)) {
    ...
    while ((count($threads) < CMN_THREADS_MAX) && ($commandsdone < count($commands))) {
      ...
      $thres[$commandsdone] = proc_open(CMN_DIR.'/cmnportcheckdo '.$commands[$commandsdone]['cmd'].' '.$commands[$commandsdone]['file'],$descriptorspec,$pipes[$commandsdone]);
...
xecho("cmnportcheck v".CMN_VERSION."\n");
...
if ($argv[1] == 'db') {
  if (file_exists(CMN_PORTCHECK_SEMAPHORE) && (posix_getpgid(intval(file_get_contents(CMN_PORTCHECK_SEMAPHORE))) !== false) ) {
    xecho("Already running (PID ".sprintf('%d',file_get_contents(CMN_PORTCHECK_SEMAPHORE)).")\n");
    die(10);
  }
  file_put_contents(CMN_PORTCHECK_SEMAPHORE,sprintf('%s',getmypid()));
}

xecho('Retrieving MN port check: ');
$result = cmn_cmd_get('/portcheck/list',array(),$response);
...
if ($argv[1] == 'nodb') {
  ...
  $result = cmn_cmd_get('/masternodes',array(),$response);
  if ($response['http_code'] == 200) {
    echo "Fetched...";
  ...
  $result = cmn_cmd_get('/masternodes',array("testnet" => 1),$response);
  if ($response['http_code'] == 200) {
    echo "Fetched...";
    $mnlist = json_decode($result,true);
    if ($mnlist === false) {
...
if (count($mnpc) > 0) {
  ...
  if (!is_dir("/dev/shm/cmnportcheck/")) {
    mkdir("/dev/shm/cmnportcheck/",0700,true);
  }
  $tmpdate = date('YmdHis');
  $commands = array();
  foreach($mnpc as $mniprow) {
    ...
    if (count($mnip) == 3) {
      ...
      $commands[] = array("status" => 0,
                          ...
                          "file" => "/dev/shm/cmnportcheck/$tmpdate.".sha1($mnip[0])."_".$mnip[1]."_".$mnip[2].".json");
    }
  }
  echo "OK (".count($commands).")\n";
  $res = cmn_portcheck_mt($commands);
...
if (count($payload) > 0) {
  ...
  $response = '';
  $content = cmn_cmd_post('/portcheck',$payload,$response);
...
if (($argc == 1) || (($argc == 2) && ($argv[1] == 'sql'))) {
  unlink(CMN_PORTCHECK_SEMAPHORE);
}
