...
define('CMN_VERSION','1.5.1');

xecho('cmnblockparser v'.CMN_VERSION."\n");
if (file_exists(CMN_BLOCKPARSER_SEMAPHORE) && (posix_getpgid(intval(file_get_contents(CMN_BLOCKPARSER_SEMAPHORE))) !== false) ) {
    xecho("Already running (PID ".sprintf('%d',file_get_contents(CMN_BLOCKPARSER_SEMAPHORE)).")\n");
    die(10);
}
file_put_contents(CMN_BLOCKPARSER_SEMAPHORE,sprintf('%s',getmypid()));

xecho('Retrieving nodes for this hub: ');
$result = cmn_cmd_get('/nodes',array(),$response);
...
$result = cmn_cmd_get('/masternodes/pubkeys',array(),$response);
if ($response['http_code'] == 200) {
  echo "Fetched...";
...
$result = cmn_cmd_get('/masternodes/donations',array(),$response);
if ($response['http_code'] == 200) {
  echo "Fetched...";
...
$result = cmn_cmd_get('/pools',array(),$response);
if ($response['http_code'] == 200) {
  echo "Fetched...";
...
$result = cmn_cmd_get('/budgetsexpected',array(),$response);
if ($response['http_code'] == 200) {
  echo "Fetched...";
...
$result = cmn_cmd_get('/superblocksexpected',array(),$response);
if ($response['http_code'] == 200) {
    echo "Fetched...";
...
function cmn_blockparse($uname, $testnet, $mnpubkeys, $mndonations, $poolpubkeys, $mnsuperblocks, $mnsuperblocks2, &$bhws, &$bws, &$btarchive, &$blockarchive, &$txarchive, &$mparchive) {
  ...
  if (!is_dir(CMN_BLOCKPARSER_ARCHIVE.$uname.'/') || !is_dir(CMN_BLOCKPARSER_ARCHIVE.$uname.'/tx/')) {
    mkdir(CMN_BLOCKPARSER_ARCHIVE.$uname.'/tx/',0700,true);
  }
  if (!is_dir(CMN_BLOCKPARSER_ARCHIVE.$uname.'/bt/')) {
    mkdir(CMN_BLOCKPARSER_ARCHIVE.$uname.'/bt/',0700,true);
  }
  echo "OK\n";
      ...
      if (substr_count($blockid,'_') == 2) {
        ...
        if (($bt !== false) && isset($bt) && array_key_exists('payee',$bt)) {
          ...
          $bhws[] = array("BlockHeight" => $blockid,
                          ...
                          "Protocol" => $btprotocol,
                          "BlockMNRatio" => $btpam);
          $btarchive["/dev/shm/$uname/bt/$btfile"] = CMN_BLOCKPARSER_ARCHIVE.$uname.'/bt/'.$btfile;
        }
    ...
    foreach($blockfiles as $blockfile) {
      ...
      if (file_exists("/dev/shm/$uname/mempool.$blockid.json")) {
        ...
        if (($mempool !== false) && isset($mempool) && (count($mempool) > 0)) {
          $mparchive["/dev/shm/$uname/mempool.$blockid.json"] = CMN_BLOCKPARSER_ARCHIVE.$uname.'/'."mempool.$blockid.json";
          foreach($mempool as $txid => $txhash) {
            if (file_exists("/dev/shm/$uname/tx/mempooltx.$blockid.$txhash.json")) {
              $txarchive["/dev/shm/$uname/tx/mempooltx.$blockid.$txhash.json"] = CMN_BLOCKPARSER_ARCHIVE . $uname . '/tx/' . "mempooltx.$blockid.$txhash.json";
              $tx = json_decode(file_get_contents("/dev/shm/$uname/tx/mempooltx.$blockid.$txhash.json"), true);
              $typeinfo = cmn_gettxtype($tx);
              if ($typeinfo['ds']) {
                $mpdscount[$blockid]++;
              }
            }
      ...
      if (($block !== false) && isset($block) && array_key_exists('height',$block)) {
        if ($block['height'] == $blockid) {
          ...
          $blockarchive["/dev/shm/$uname/$blockfile"] = CMN_BLOCKPARSER_ARCHIVE.$uname.'/'.$blockfile;
          $gentx = false;
          $dscount = 0;
          foreach($block['tx'] as $txid => $txhash) {
            if (in_array("transaction.$txhash.json",$txfiles)) {
              $txarchive["/dev/shm/$uname/tx/transaction.$txhash.json"] = CMN_BLOCKPARSER_ARCHIVE.$uname.'/tx/'."transaction.$txhash.json";
...
function cmn_ws_submit($bhws, $bws, $blockarchive, $btarchive, $txarchive, $mparchive)
{
    if ((count($bhws) > 0) || (count($bws) > 0)) {
        ...
        $response = '';
        $content = cmn_cmd_post('/blocks', $payload, $response);
...
foreach($nodes as $node) {
  if ($node['NodeTestNet'] == "0") {
      cmn_blockparse($node['NodeName'], $node['NodeTestNet'], $mnpubkeys, $mndonations, $poolpubkeys, $mnsuperblocks, $mnsuperblocks2, $bhws, $bws, $btarchive, $blockarchive, $txarchive, $mparchive);
  }
}
cmn_ws_submit($bhws, $bws, $blockarchive, $btarchive, $txarchive, $mparchive);
...
unlink(CMN_BLOCKPARSER_SEMAPHORE);
