...
/*
    ...
    https://github.com/terracoin/chcninja-ctl
...
xecho("cmnbalance v".CMN_VERSION."\n");
if (file_exists(CMN_BALANCE_SEMAPHORE) && (posix_getpgid(intval(file_get_contents(CMN_BALANCE_SEMAPHORE))) !== false) ) {
  xecho("Already running (PID ".sprintf('%d',file_get_contents(CMN_BALANCE_SEMAPHORE)).")\n");
  die(10);
}
file_put_contents(CMN_BALANCE_SEMAPHORE,sprintf('%s',getmypid()));

xecho('Retrieving MN info (mainnet): ');
$result = cmn_cmd_get('/masternodes',array("testnet"=>0),$response);
...
$result = cmn_cmd_get('/balances',array(),$response);
if ($response['http_code'] == 200) {
  echo "Fetched...";
...
foreach($mnpubkeys as $mnpubkey) {
  if (array_key_exists($mnpubkey,$mnlastupdate)) {
    ...
    if ((($argc > 1) && ($argv[1] == 'force')) || ($delta > CMN_BALANCE_INTERVAL)) {
      $mncheck[] = $mnpubkey;
    }
    else {
      $mnchecknot[] = $mnpubkey;
...
foreach($mncheck as $mnpubkey) {
  ...
  $url = str_replace('%%p%%',$mnpubkey,CMN_BALANCE_URL_MAINNET);
  ...
  if ($res === false) {
    ...
    $mncurbalancerdisplay = sprintf("%.9f",$mncurbalance/100000000);
    echo "$mncurbalancerdisplay CHC\n";
    $payload[] = array('TestNet' => 0,
                       'PubKey' => $mnpubkey,
                       'Balance' => $mncurbalancerdisplay,
...
foreach($tncheck as $mnpubkey) {
  ...
  $url = str_replace('%%p%%',$mnpubkey,CMN_BALANCE_URL_TESTNET);
...
if (count($payload) > 0) {
  ...
  $response = '';
  $content = cmn_cmd_post('/balances',$payload,$response);
